...
if($branch -match $pullrequest) {        
    ...
    Write-Output "Cannot find pull request ID getting source branch from branch parameter"
    $branchref = "refs/heads/(?<realBranchname>.*)"
    if($branch -match $branchref) {        
        $branchname = $Matches.realBranchname;
        Write-Output "Real source branch is $branchname"
    }
    else { 
        Write-Output "Cannot find real source branch" 
