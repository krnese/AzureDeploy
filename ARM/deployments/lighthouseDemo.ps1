...
Search-AzGraph -Query "summarize count() by tenantId" -Subscription (Get-AzSubscription).SubscriptionId | ConvertTo-Json

Search-AzGraph -Query "where type =~ 'Microsoft.Storage/storageAccounts' | project name, location, subscriptionId, tenantId, properties.supportsHttpsTrafficOnly" -Subscription (Get-AzSubscription).SubscriptionId | convertto-json
